      ...
      repeat(1) @(vif.ref_cb)
       begin 
        if(ref_trans.write_enb && ref_trans.read_enb)
           ref_trans.data_out=0;
        else if(ref_trans.write_enb) begin
         MEM[ref_trans.address]=ref_trans.data_in;
        $display("REFERENCE MODEL DATA IN MEMORY=%d",MEM[ref_trans.address],$time);
             end
        else if(ref_trans.read_enb) begin
         ref_trans.data_out=MEM[ref_trans.address];
        $display("REFERENCE MODEL DATA OUT FROM MEMORY=%d",ref_trans.data_out,$time);
            end
       end
